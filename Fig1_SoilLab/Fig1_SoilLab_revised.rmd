...
nutrients5<-aggregate(Variable~Experimental_Year+Tillage+Fertilization+Nutrient, nutrients4, mean, na.rm=TRUE)

plot1 = ggplot() +
  geom_bar(data=nutrients5, 
           aes(x = as.factor(Experimental_Year), y=Variable, fill=paste0(Tillage,"-",Fertilization)), 
           stat="identity", position=position_dodge(width=0.9), alpha=0.6) +
  geom_point(data=nutrients4, 
             aes(x=as.factor(Experimental_Year), y=Variable, fill=paste0(Tillage,"-",Fertilization)), 
             position=position_jitterdodge(jitter.width=0.2, dodge.width=0.9), shape=21,
             alpha=0.7, size=2) +
...
```
Create q-q-plots for total nitrogene and SOM 
```{r}
