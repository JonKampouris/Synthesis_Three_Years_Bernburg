...
for(i in unique(paste0(nutrients4$Nutrient))){
  ...
  singular_check <- isSingular(file2)
  print(singular_check)  
  variances <- VarCorr(file2)
  print(variances)  
...
Create the mixed effect model function with year as fixed effect 
```{r}
# mixed_effects_fixed =list()
# for(i in unique(paste0(nutrients4$Nutrient))){
#   file1=dplyr::filter(nutrients4, Nutrient==paste0(i))
#   file1$G=paste0(file1$Fertilization,";",file1$Tillage)
#   file1$Experimental_Year=as.factor(file1$Experimental_Year)
#   file1$Variable=scale(file1$Variable)
#   file2=lm(Variable~TILLAGE2*FERTILIZATION2+Experimental_Year, data=file1)
# 
#   #Apply Bootstrap for inferring p-values and 95% CI
#   boot.out = boot(file2, boot_fn, seed=11122022, R=10000, parallel = "multicore", ncpus = 20)
#   print("Boot  finished")
# 
# 
#   CI.lower = apply(boot.out$t, 2, function(x) as.numeric(quantile(x, probs=.025, na.rm=TRUE)))
...
# 
# 
#   mixed_effects_fixed=rbind(mixed_effects_fixed,file5)
# }
# 
# mixed_effects_fixed2=filter(mixed_effects_fixed, Variable!="(Intercept)")
# 
# mixed_effects_fixed2$pval[mixed_effects_fixed2$pval<0.05]="*p* < 0.05"
# mixed_effects_fixed2$pval[mixed_effects_fixed2$pval!="*p* < 0.05"]="*p* > 0.05"
```

Create the proper labels for the graph.
```{r}
# mixed_effects_fixed2$Nutrient=factor(
#   mixed_effects_fixed2$Nutrient,
#   levels = c("OM[%]","N[%]"),
#   labels = c( expression(paste("SOM (%)")),
#               expression(paste("N (%)"))
#               ))
# 
# mixed_effects_fixed2$Variable=gsub("TILLAGE2:FERTILIZATION2",
#   "Interaction",mixed_effects_fixed2$Variable)
# 
# mixed_effects_fixed2$Variable=gsub("FERTILIZATION2",
#   "Ext. vs Int. N-Fert.",
#   gsub("TILLAGE2", "CT vs MP Tillage", mixed_effects_fixed2$Variable)
# )
# 
# mixed_effects_fixed2$Variable=factor(mixed_effects_fixed2$Variable,
# levels = c ("Interaction",   "Ext. vs Int. N-Fert." , "CT vs MP Tillage"              
# ))
# 
# plot2_fixed =ggplot(mixed_effects_fixed2, 
#   aes(y=Variable, x=bootMed, fill=pval)) +
#   geom_pointrange(shape=21, size=3, aes(xmax=upper, xmin= lower)) +
#   facet_wrap(~Nutrient, scales="fixed", ncol=1, labeller=label_parsed) +
#   scale_fill_manual(values = c("#f1a340","#f7f7f7"), name="p-value") +
...
#   xlim(-2, 2)
# 
# png(filename = "Fig1_SoilLab_Year_revised.png",
#     width = 8000, 
#     height = 3000, 
#     res=300)
# print(ggarrange(plot2, plot2_fixed,
#                 labels = c("A)","B)"),
...
plot1= ggplot(nutrients4, 
  ...
  geom_boxplot() +
   geom_point(position=position_dodge(width=0.75), shape=21, size=5, alpha=0.7, aes(fill=paste0(Tillage,"-", Fertilization))) +
  #geom_jitter(position = position_jitterdodge(dodge.width = 0.75), size = 1.5, alpha = 0.7) +
...
data_TN <- nutrients4 %>% filter(Nutrient=='paste("N (%)")') %>% pull(Variable) 
data_SOM <- nutrients4 %>% filter(Nutrient=='paste("SOM (%)")') %>% pull(Variable) 

# Speichern der Plots als PNG
png("QQ_Plots.png", width = 1200, height = 600)

# Zwei Plots nebeneinander anordnen
par(mfrow = c(1, 2))

# Erstellen des ersten Q-Q-Plots
qqnorm(data_TN, main = "Q-Q Plot for N (%)")
qqline(data_TN, col = "red")

# Erstellen des zweiten Q-Q-Plots
qqnorm(data_SOM, main = "Q-Q Plot for SOM (%)")
qqline(data_SOM, col = "red")

# Speichern und Schließen der PNG-Datei
dev.off()
```
Exploration Plots 
```{r}
# dataFrame_SOM <- nutrients4 %>% filter(Nutrient=='paste("SOM (%)")')
# dataFrame_TN <- nutrients4 %>% filter(Nutrient=='paste("N (%)")')
# 
# plotExp_SOM = ggplot(dataFrame_SOM, aes(x=interaction(Tillage, Fertilization), y = Variable, fill = interaction(Tillage, Fertilization))) +
#   geom_violin() + 
#   facet_wrap(~ Experimental_Year) + 
#   labs(x="Treatments", y = "SOM (%)", fill = "Treatment") +
#   scale_fill_manual(values = c("#c8e9df","#4ab795", "#fdb79c",  "#fb6328"), name="") +
#   theme_minimal() +  # Ein einfaches, sauberes Theme
#   theme(axis.text.x=element_text(angle=45, hjust=1)) 
# 
# plotExp_TN = ggplot(dataFrame_TN, aes(x=interaction(Tillage, Fertilization), y = Variable, fill = interaction(Tillage, Fertilization))) +
#   geom_violin() + 
#   facet_wrap(~ Experimental_Year) + 
#   labs(x="Treatments", y = "SOM (%)", fill = "Treatment") +
#   scale_fill_manual(values = c("#c8e9df","#4ab795", "#fdb79c",  "#fb6328"), name="") +
...
#     legend = "top")
# 
# png("Exploration_Plots.png", width = 1200, height = 600)
# print(plots)
# dev.off()
